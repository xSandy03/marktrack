...
return view.extend({
            ...
            if (activityColumnIndex >= 0) {
                ...
                dataRows.forEach(function(row) {
                    ...
                    if (cells[activityColumnIndex] && sectionId) {
                        var ruleName = uci.get('marktrack', sectionId, 'name');
                        var counterEnabled = uci.get('marktrack', sectionId, 'counter');
    ...
    handleSaveApply: function(ev) {
        return this.handleSave(ev)
            ...
            .then(() => uci.load('marktrack'))
            .then(() => uci.get_first('marktrack', 'global', 'enabled'))
            .then(enabled => {
                if (enabled === '0') {
                    return fs.exec_direct('/etc/init.d/marktrack', ['stop']);
                } else {
                    return fs.exec_direct('/etc/init.d/marktrack', ['restart']);
                }
            })
            .then(() => {
                ui.hideModal();
    ...
    render: function() {
        ...
        o.cfgvalue = function(section_id) {
            var proto = uci.get('marktrack', section_id, 'proto');
        ...
        o.cfgvalue = function(section_id) {
            var value = uci.get('marktrack', section_id, 'proto');
        ...
        o.write = function(section_id, value) {
            if (value && value.length) {
                uci.set('marktrack', section_id, 'proto', value.join(' '));
            } else {
                uci.unset('marktrack', section_id, 'proto');
            }
        };
        ...
        o.write = function(section_id, formvalue) {
            ...
            uci.set('marktrack', section_id, 'enabled', formvalue);
        };
        o.load = function(section_id) {
            var value = uci.get('marktrack', section_id, 'enabled');
        ...
        o.cfgvalue = function(section_id) {
            var counterEnabled = uci.get('marktrack', section_id, 'counter');
            if (counterEnabled !== '1') {
                return '-';
            }
            return 'loading...';
        ...
        return m.render().then(function(rendered) {
            ...
            return E('div', {}, [ renderSectionNav('rules'), rendered ]);
        });
    },
